...
def main() -> None:
    ...
    sm = cm.ScalarMappable(cmap="PiYG", norm=norm) # PiYG asigna Rosa (+) y Verde (-)
    sm.set_array([])
    fig.colorbar(sm, ax=ax, label="Inner Product Score (Green: Anti-development | Pink: Pro-development)")
    plt.tight_layout()
